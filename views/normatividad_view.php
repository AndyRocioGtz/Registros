<div class="container">
  <div class="in">
    <div id="portfolio" class="container-fluid text-center bg-grey">
      <h2>Normatividad</h2>
      <div>
        <a href="../views/dirdgrpp.php">
        <button type="button" class="btn btn-success">Regresar a Dir.Registros Públicos</button></a>
      </div>
      <p class="text-justify p">
      </p>
      <br>
      <br>
      <br>
      <div class="row text-center slideanim">
        <div class="col-sm-4">
          <div class="thumbnail">
            <a href="../files/nrrpp2012.pdf" target="_blank">
              <img src="../dist/images/reglamento.jpg" alt="CDGTO" width="400" height="300">
              <p><strong>Reglamento RPP GTO. 2012 (Vigente)</strong></p>
            </a>
          </div>
        </div>
        <div class="col-sm-4">
          <div class="thumbnail">
            <a href="../files/rrppab.pdf" target="_blank">
              <img src="../dist/images/reglamentos-696x400.png" alt="Reglamento" width="400" height="300">
              <p><strong>Reglamento del Registro Público de la Propiedad (Abrogado)</strong></p>
            </a>
          </div>
        </div>
        <div class="col-sm-4">
          <a href="../files/1.pdf" target="_blank">
          <div class="thumbnail">
            <img src="../dist/images/infonavit.jpg" alt="Lineamientos" width="200" height="300">
            <p><strong>Lineamientos</strong></p>
            <p>Operación del Registro Público de la Propiedad</p>
          </div>
          </a>
        </div>
      </div><br>
        <!-- SALTO -->
      <div class="row text-center slideanim">
        <div class="col-sm-4">
          <div class="thumbnail">
            <a href="../files/Ley_de_Ingresos_para_el_Estado_de_Guanajuato_para_el_Ejercicio_Fiscal__2016.pdf" target="_blank">
              <img src="../dist/images/AAADinero_0.jpg" alt="ingresos" width="280" height="150">
              <p><strong>Ley de Ingresos</strong></p>
              <p>Del Estado de Guanajuato</p>
            </a>
          </div>
        </div>

        <div class="col-sm-4">
          <div class="thumbnail">
            <a href="../files/criteriosregistrales.pdf" target="_blank">
              <img src="../dist/images/admin_alma.png" alt="ingresos" width="200" height="150">
              <p><strong>Criterios Registrales</strong></p>
              <p>Del Estado de Guanajuato</p>
            </a>
          </div>
        </div>

        <div class="col-sm-4">
          <a href="../files/acuerdo-administrativo.pdf" target="_blank">
          <div class="thumbnail">
            <img src="../dist/images/acuerdo.png" alt="acuerdo" width="150" height="100">
            <p><strong>Acuerdo Administrativo cobro de Productos</strong></p>
            <p>Criterios Registrales</p>
          </div>
          </a>
        </div>
      </div><br>
      
    </div>
    
</div> <!-- IN -->
</div><!-- CONTAINER -->